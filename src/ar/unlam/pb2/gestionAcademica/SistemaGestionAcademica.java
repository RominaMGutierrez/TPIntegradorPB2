...
public class SistemaGestionAcademica {
	...
	public Boolean hayFechasTraspuestas(CicloLectivo nuevo) {
		for(int i = 0; i<ciclosLectivos.size(); i++) {
			if(nuevo.getFechaDeInicio().isBefore(ciclosLectivos.get(i).getFechaDeFinalizacion()) || nuevo.getFechaDeInicio().isEqual(ciclosLectivos.get(i).getFechaDeFinalizacion())) {
				return true;
			}
		}
		return false;
	...
	public Boolean agregarComision(Comision comision) {
		...
		for(int i = 0; i<comisiones.size(); i++) {
			if(comisiones.get(i).getMateria().equals(nuevaMateria) || comisiones.get(i).getCicloLectivo().equals(nuevoCiclo) || comision.getTurno().toLowerCase().equals(comisiones.get(i).getTurno().toLowerCase() )){
				repetido = true;
			}
		}
		if(repetido) {
	...
	public Boolean registrarNota(Integer dniAlumno, Integer codigoDeComision, Integer notaAlumno, TipoDeParcial tipoParcial) {
		Alumno alumno = buscarAlumno(dniAlumno);
	...
	public List<Materia> obtenerMateriasAprobadasParaUnAlumno(Integer dniAlumno) {
		...
		for(RegistroAcademico registro : alumno.getMateriasAprobadas()) {
			materiasAprobadas.add(registro.getMateria());
		}
		return materiasAprobadas;
	}
	
	public Integer obtenerNotaDeAlumno(Integer dniAlumno, Integer codigoMateria) {
		Alumno alumno = buscarAlumno(dniAlumno);
		for(RegistroAcademico registro : alumno.getMateriasAprobadas()) {
			if(registro.getMateria().getCodidoDeMateria().equals(codigoMateria)) {
				return registro.getNotaFinal();
			}
		}
		return 0;
	}
	...
	public Integer calcularPromedio(Integer dniAlumno) {
		Alumno alumno = buscarAlumno(dniAlumno);
		Integer sumaNotas = 0;
		Integer cantidadDeMaterias = 0;
		for(RegistroAcademico registro : alumno.getMateriasAprobadas()) {
			sumaNotas = registro.getNotaFinal();
			cantidadDeMaterias++;
		}
		if(alumno.getMateriasAprobadas().equals(null)) {
			return 0;
		}else {
			
		}
		return sumaNotas/cantidadDeMaterias;
	}
}